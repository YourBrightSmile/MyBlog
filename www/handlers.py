...
'''
     编写Web API封装web App的功能
     通过API操作数据，将前端和后端的代码隔离，使后端代码易于测试，前端代码编写更简单
     一个API也是一个URL处理函数，API直接返回数据而非网页（这里使用JSON封装数据）
'''
#@get('/api/users')
...
_RE_NAME= re.compile(r'^[\u4E00-\u9FFFa-zA-Z0-9]{3,12}$')
@post('/api/register') 
async def api_register_user(*,email,name,passwd):
    ...
    if len(users)>0:
        raise APIError('register:faile','eamil','Email is already in use')
    uid = next_id()
    ...
    #session cookie
    r = web.Response()
    r.set_cookie(COOKIE_NAME,user2cookie(user,86300),max_age=86400,httponly=True)
    user.passwd='*******'
